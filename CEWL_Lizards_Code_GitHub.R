...
# Run this script from anywhere inside the GitHub repository.
# The here package locates the repository root using the .Rproj file, .git folder,
# or a .here file. Do not use setwd() or machine-specific absolute paths.
...
data_dir <- here("Data")
worldclim_dir <- here("Data", "World_Clim")
...
# Helper to stop with a clear message when a required file is missing
...
# Helper to verify that monthly raster searches returned files
require_rasters <- function(paths, variable_name, expected = 12) {
  if (length(paths) != expected) {
    search_dir <- if (length(paths) > 0) dirname(paths[1]) else worldclim_dir
    stop(
      "Expected ", expected, " ", variable_name, " raster files but found ",
      length(paths), ". Check: ", search_dir,
      call. = FALSE
    )
  }
  paths
...
individual_data_file <- require_file(here("Data", "df3_final.csv"))
site_data_file <- require_file(here("Data", "data-location.csv"))

dat <- read.csv(individual_data_file)
location_dat1 <- read.csv(site_data_file)
...
precip_files <- list.files(
  here("Data", "World_Clim", "prec"),
  pattern = "wc2.1_30s_prec_.*\\.tif$",
  full.names = TRUE
)
precip_files <- require_rasters(precip_files, "precipitation")
...
srad_files <- list.files(
  here("Data", "World_Clim", "srad"),
  pattern = "wc2.1_30s_srad_.*\\.tif$",
  full.names = TRUE
)
srad_files <- require_rasters(srad_files, "solar-radiation")
...
vapr_files <- list.files(
  here("Data", "World_Clim", "vapr"),
  pattern = "wc2.1_30s_vapr_.*\\.tif$",
  full.names = TRUE
)
vapr_files <- require_rasters(vapr_files, "water-vapor-pressure")
...
tmax_files <- list.files(
  here("Data", "World_Clim", "tmax"),
  pattern = "wc2.1_30s_tmax_.*\\.tif$",
  full.names = TRUE
)
tmax_files <- require_rasters(tmax_files, "maximum-temperature")
...
tmin_files <- list.files(
  here("Data", "World_Clim", "tmin"),
  pattern = "wc2.1_30s_tmin_.*\\.tif$",
  full.names = TRUE
)
tmin_files <- require_rasters(tmin_files, "minimum-temperature")
...
tavg_files <- list.files(
  here("Data", "World_Clim", "tavg"),
  pattern = "wc2.1_30s_tavg_.*\\.tif$",
  full.names = TRUE
)
tavg_files <- require_rasters(tavg_files, "average-temperature")
...
write.csv(CEWL_table, file.path(table_dir, "Table_CEWL_habitat_model.csv"), row.names = FALSE)
write.csv(CEWL_pairwise, file.path(table_dir, "Table_CEWL_emmeans_pairwise.csv"), row.names = FALSE)
...
print(CEWL_plot)
ggsave(file.path(figure_dir, "Figure_CEWL_by_habitat.pdf"), CEWL_plot,
...
write.csv(micro_pca_loadings, file.path(table_dir, "Table_microclimate_PCA_loadings.csv"), row.names = FALSE)
write.csv(macro_pca_loadings, file.path(table_dir, "Table_macroclimate_PCA_loadings.csv"), row.names = FALSE)
write.csv(micro_pca_variance, file.path(table_dir, "Table_microclimate_PCA_variance.csv"), row.names = FALSE)
write.csv(macro_pca_variance, file.path(table_dir, "Table_macroclimate_PCA_variance.csv"), row.names = FALSE)
write.csv(MicroModel_table, file.path(table_dir, "Table_microPC1_macroPC1_model.csv"), row.names = FALSE)
write.csv(MicroModel_anova, file.path(table_dir, "Table_microPC1_macroPC1_ANOVA.csv"), row.names = FALSE)
...
print(MacroPCA_plot)
ggsave(file.path(figure_dir, "Figure_macroclimate_PCA.pdf"), MacroPCA_plot,
...
print(MicroPCA_plot)
ggsave(file.path(figure_dir, "Figure_microclimate_PCA.pdf"), MicroPCA_plot,
       width = 6, height = 5, units = "in")
...
ggsave(file.path(figure_dir, "Figure_macroPC1_vs_microPC1.pdf"), MacroMicro_plot,
       width = 6, height = 5, units = "in")
...
ggsave(file.path(figure_dir, "Figure_combined_PCA_panels.pdf"), PCA_combined_plot,
...
write.csv(CEWL_env_table, file.path(table_dir, "Table_CEWL_microclimate_model.csv"), row.names = FALSE)
write.csv(CEWL_env_anova, file.path(table_dir, "Table_CEWL_microclimate_ANOVA.csv"), row.names = FALSE)
write.csv(CEWL_env_vif, file.path(table_dir, "Table_CEWL_microclimate_VIF.csv"), row.names = FALSE)
...
print(CEWL_VPD_plot)
ggsave(file.path(figure_dir, "Figure_CEWL_vs_VPD.pdf"), CEWL_VPD_plot,
...
ggsave(file.path(figure_dir, "Figure_descriptive_microclimate_densities.pdf"),
...
if (make_environmental_map) {
  total_precip <- raster(require_file(here("Data", "World_Clim", "total_precipitation.tif")))
  ...
  Cairo::CairoPDF(file.path(figure_dir, "Figure_environmental_maps.pdf"), width = 7.5, height = 10)
...
writexl::write_xlsx(supplement_tables, file.path(table_dir, "Supplementary_analysis_tables.xlsx"))

message("Analysis complete. Figures written to: ", figure_dir,
